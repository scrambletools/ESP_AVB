...
static int avb_initialize_state(avb_state_s *state, avb_config_s *config) {
  ...
  avtp_stream_format_aaf_pcm_s sf3 =
      AVB_DEFAULT_FORMAT_AAF(16, aaf_pcm_sample_rate_44_1k, 8, false);
  avtp_stream_format_aaf_pcm_s sf4 =
      AVB_DEFAULT_FORMAT_AAF(24, aaf_pcm_sample_rate_44_1k, 8, false);
  avtp_stream_format_aaf_pcm_s sf5 =
      AVB_DEFAULT_FORMAT_AAF(24, aaf_pcm_sample_rate_48k, 8, false);
  avtp_stream_format_aaf_pcm_s sf6 =
      AVB_DEFAULT_FORMAT_AAF(24, aaf_pcm_sample_rate_96k, 8, false);
  ...
  if (state->config.listener) {
    ...
    for (int i = 0; i < state->num_input_streams; i++) {
      ...
      if (state->input_streams[i].connected &&
        ...
        avb_send_msrp_listener(state, mrp_attr_event_join_mt,
...
static void avb_stream_in_task(void *task_param) {
  ...
  struct pollfd poll_fd = {.events = POLLIN, .fd = stream_in_params->l2if};
  ...
  while (1) {
    ...
    if (poll(&poll_fd, 1, stream_in_params->interval / 1000)) {
      ...
      if (poll_fd.revents) {
        ...
        ret = avb_net_recv(stream_in_params->l2if, stream_in_data,
                           stream_in_params->buffer_size, &ts, &src_addr);
        if (ret <= 0) {
        ...
        if ((aaf_msg->subtype == avtp_subtype_aaf) &&
            (memcmp(aaf_msg->stream_id, stream_in_params->stream_id,
                    UNIQUE_ID_LEN) == 0)) {
          memcpy(aaf_msg->stream_id, stream_in_params->stream_id,
                 UNIQUE_ID_LEN);
          int ret = i2s_channel_write(
              stream_in_params->i2s_tx_handle, stream_in_data,
              stream_in_params->buffer_size, &bytes_write, timeout_ms);
          if (ret != ESP_OK) {
            avberr("I2S: write failed, %s",
                   ret == ESP_ERR_TIMEOUT ? "timeout" : "unknown");
            goto err;
          }
          if (bytes_read != bytes_write) {
            avbwarn("I2S: %d bytes read but only %d bytes are written",
